// src/pages/equipment/EquipmentReportPreview.jsx
...
export default function EquipmentReportPreview() {
  ...
  const derived = useMemo(() => {
    ...
    const mtbf = Number(payload?.avg_life) || null; // 평균수명(시간) = MTBF 근사
  ...
  /** ── 실무형 보고서 프레임(미리보기 탭에서 llm 내용 부족시 보완 출력) ── */
  const fallbackReport = useMemo(() => {
    const lines = [];
    lines.push(`■ 장비 개요`);
    lines.push(
      `- 분류: ${LABELS[category]} / 장비명: ${payload?.name ?? "-"} / 제조사: ${
        payload?.manufacturer ?? "-"
      }`
    );
    lines.push(
      `- 보호등급: ${payload?.protection_rating ?? "-"} / 내용연수: ${
        payload?.service_years ?? "-"
      }년`
    );
    lines.push("");
    ...
    if (payload?.repair_cost)
      lines.push(`- 최근 수리 비용: ${fmtKRW(payload.repair_cost)}`);
    ...
    if (derived.failureYr != null)
      lines.push(`- 연간 예상 고장 횟수: ${fmtNum(derived.failureYr, "회")}`);
    lines.push("");

    lines.push(`■ 리스크 및 권고`);
    lines.push(
      `- 노후화/환경 요소(비·염분·온도)에 따라 유지보수 주기를 조정하세요. ` +
        `고장 기록이 증가 시 예방점검 주기를 단축 권고.`
    );
    lines.push(
      `- 예비 부품(램프/모듈/패널) 최소 ${LABELS[category] === "표지" ? "2세트" : "3개"} 이상 확보 권고.`
    );
    lines.push("");
    ...
    lines.push(
      `- 인력/시간 가정: 고장 1회당 수리 ${fmtNum(payload?.repair_time, "시간")}, ` +
        `시간당 인건비 ${fmtKRW(payload?.labor_rate)}.`
    );
    lines.push("");
  ...
  /** 본문 미리보기 내용 */
  const previewText =
    (llmText && llmText.trim().length > 0 ? llmText : fallbackReport);

  if (!category || !payload) {
    return (
      <div style={{ padding: 24 }}>
        미리보기 입력이 없습니다. 이전 화면에서 다시 시도하세요.
      </div>
    );
  }
    ...
    <>
      ...
      <div style={styles.page}>
        <div style={styles.shell} className="shell">
          ...
          {/* ===== Left: Report body (tabs) ===== */}
          <div style={styles.card}>
            <div style={styles.tabRow} className="no-print">
              <button
                type="button"
                style={styles.tab(tab === "preview")}
              ...
              {tab === "edit" && (
                <>
                  ...
                  <div className="no-print" style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 10 }}>
                    <button style={styles.btn} onClick={() => setLlmText(fallbackReport)}>
                      기본 프레임으로 대체
                    </button>
                    <button style={styles.btn} onClick={() => setLlmText("")}>
                      비우기
                    </button>
                  </div>
                </>
                ...
                <>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <div style={{ color: "#6b7280", fontSize: 13 }}>
                      제조사: {payload?.manufacturer || "-"} · 보호등급:{" "}
                      {payload?.protection_rating || "-"} · 내용연수:{" "}
                      {fmtNum(payload?.service_years, "년")}
                    </div>
                  </div>
            ...
            >
              <button style={styles.btn} onClick={onCancel} disabled={loading}>
                취소
              </button>
              <button style={styles.btn} onClick={() => setTab("preview")}>
                미리보기
              </button>
              <button style={styles.btn} onClick={() => setTab("edit")}>
                편집
              </button>
              <button style={styles.btn} onClick={onReset} disabled={loading}>
                초기화
              </button>
              <button style={styles.btnPrimary} onClick={onSave} disabled={loading}>
                {loading ? "불러오는중..." : "저장"}
              </button>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={styles.card}>
              <div style={styles.rightCardBody}>
                ...
                <div style={styles.kv}>
                  <div style={styles.k}>최근 수리 비용</div>
                  <div style={styles.v}>{fmtKRW(payload?.repair_cost)}</div>
                  <div style={styles.k}>최근 수리 시간</div>
                  <div style={styles.v}>{fmtNum(payload?.repair_time, "시간")}</div>
                  <div style={styles.k}>시간당 인건비</div>
                  <div style={styles.v}>{fmtKRW(payload?.labor_rate)}</div>
                  <div style={styles.k}>MTBF(평균고장간격)</div>
                  <div style={styles.v}>{fmtNum(derived.mtbf, "시간")}</div>
                  <div style={styles.k}>연간 예상 고장 횟수</div>
                  <div style={styles.v}>{fmtNum(derived.failureYr, "회")}</div>
                </div>
              </div>
            </div>
            ...
            <div style={styles.card}>
              <div style={styles.rightCardBody}>
                ...
                <div style={styles.kv}>
                  <div style={styles.k}>분류</div>
                  <div style={styles.v}>{LABELS[category]}</div>
